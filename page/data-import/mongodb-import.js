...
// npm install
//
// Then, run the following command to actually perform the data import:
// node mongodb-import.js
//
// Observe the console.log() output and ensure that there were no errors.
// Obersve that this process may take a few seconds to complete (just wait for it to finish).
//
...
// (sequentially, i.e., import the second data set only after the first one has finished, and so on)
...
.finally(() => {
    ...
    .catch((error) => {
        fs.writeFile("./import-error.txt", error.toString(), (err) => {
